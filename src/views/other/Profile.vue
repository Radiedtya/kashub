    ...
    <div v-if="loading" class="space-y-6 animate-pulse">
      ...
      <div class="bg-white border border-zinc-200 rounded-xl p-6 flex flex-col sm:flex-row items-center gap-6">
        ...
        <div class="flex-1 text-center sm:text-left space-y-3">
          <div class="flex flex-col sm:flex-row items-center gap-2 justify-center sm:justify-start">
            ...
            <div class="h-5 w-20 bg-zinc-200 rounded-full"></div>
          </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div class="bg-white border border-zinc-200 rounded-xl p-6 space-y-4">
          <div class="h-5 w-40 bg-zinc-200 rounded"></div>
          <div class="h-3 w-56 bg-zinc-200 rounded"></div>
          <div class="space-y-4 pt-2">
            <div v-for="i in 4" :key="i">
              <div class="h-3 w-24 bg-zinc-200 rounded mb-2"></div>
              <div class="h-10 w-full bg-zinc-200 rounded-lg"></div>
            </div>
          </div>
          <div class="flex justify-end pt-2">
            <div class="h-10 w-32 bg-zinc-200 rounded-lg"></div>
          </div>
        </div>
        <div class="bg-white border border-zinc-200 rounded-xl p-6 space-y-4">
          <div class="h-5 w-40 bg-zinc-200 rounded"></div>
...
onMounted(() => {
  // Delay 1 detik biar skeleton loading-nya sempat kelihatan
  setTimeout(() => {
    initForm();
    loading.value = false;
  }, 1000);
