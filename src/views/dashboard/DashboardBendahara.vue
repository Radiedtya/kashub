    ...
    <div v-else-if="dashboardData">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div class="stat-card bg-white border border-zinc-200 rounded-xl p-5">
          ...
          <p class="text-2xl font-bold text-zinc-900">Rp {{ formatRupiah(animatedMasuk) }}</p>
        ...
        <div class="stat-card bg-white border border-zinc-200 rounded-xl p-5">
          ...
          <p class="text-2xl font-bold text-zinc-900">Rp {{ formatRupiah(animatedKeluar) }}</p>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-5 mt-5">
        ...
        <div class="chart-card bg-white border border-zinc-200 rounded-xl p-6 flex flex-col">
          ...
          <div class="flex-1 flex items-center justify-center relative my-4 h-56">
      ...
      <div class="bottom-card bg-white border border-zinc-200 rounded-xl p-6 mt-5">
        <div class="flex items-center justify-between mb-5">
          <div>
            <h2 class="text-base font-semibold text-zinc-800">Antrian Konfirmasi Transaksi</h2>
            <p class="text-zinc-400 text-xs">Siswa yang menunggu pembayaran dikonfirmasi</p>
          </div>
        </div>
        <div class="flex flex-col divide-y divide-zinc-100">
          <div v-for="trx in dashboardData.transaksi_pending_list" :key="trx.id" class="flex items-center gap-3 py-3">
            ...
            <div class="flex-1 min-w-0">
              ...
              <p class="text-zinc-400 text-[10px]">Iuran {{ trx.iuran?.bulan }}/{{ trx.iuran?.tahun }} · {{ formatDate(trx.created_at) }}</p>
            </div>
            <div class="text-right shrink-0">
              <p class="font-semibold text-zinc-700 text-xs">Rp {{ formatRupiah(trx.jumlah) }}</p>
              <button @click="confirmAction(trx)" class="text-[10px] font-semibold text-blue-600 hover:underline transition">Konfirmasi Sekarang</button>
            </div>
          </div>
          <p v-if="dashboardData.transaksi_pending_list.length === 0" class="text-center text-zinc-400 text-sm py-8">Tidak ada antrian transaksi</p>
        </div>
      </div>
    </div>
  </div>
...
const formatRupiah = (angka) =>
  new Intl.NumberFormat("id-ID", { maximumFractionDigits: 0 }).format(angka || 0);
...
const triggerAnimations = () => {
  ...
  // 3. Bottom Card: Slide dari kanan
  anime({
    targets: '.bottom-card',
    translateX: [50, 0],
    opacity: [0, 1],
